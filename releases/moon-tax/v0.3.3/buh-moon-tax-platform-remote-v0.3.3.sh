...
TAX_WHEEL="aa_buh_moon_tax-0.3.2-py3-none-any.whl"
...
TAX_SHA256="12fe8d278398f4aa270fb0f88e2e09b8556c1525d4a1c138c2a4663382ec4e85"
...
cleanup() {
    case "$0" in
        /tmp/buh-moon-tax-platform-remote-v0.3.3.sh)
            rm -f -- "$0" \
...
COPY conf/aa_buh_moon_tax-0.3.2-py3-none-any.whl /tmp/
RUN python3 -m pip install --no-cache-dir --upgrade \\
    ...
    /tmp/aa_buh_moon_tax-0.3.2-py3-none-any.whl
...
verify_image_versions() {
    ...
    expected="5.2.0|9.6.0|5.0.4|4.0.3|3.1.0.post1|0.3.0|0.2.0|0.3.2"
    for service in "${AUTH_SERVICES[@]}"; do
        output="$({
...
install_or_update() {
    ...
    install -o root -g root -m 0644 "/tmp/${STRUCTURES_WHEEL}" \
        "${APP_DIR}/conf/${STRUCTURES_WHEEL}"
    install -o root -g root -m 0644 "/tmp/${MOON_WHEEL}" \
        "${APP_DIR}/conf/${MOON_WHEEL}"
    install -o root -g root -m 0644 "/tmp/${OPS_WHEEL}" \
        "${APP_DIR}/conf/${OPS_WHEEL}"
    install -o root -g root -m 0644 "/tmp/${ANALYTICS_WHEEL}" \
        "${APP_DIR}/conf/${ANALYTICS_WHEEL}"
    install -o root -g root -m 0644 "/tmp/${TAX_WHEEL}" \
        "${APP_DIR}/conf/${TAX_WHEEL}"
    ...
    echo "Row Accept and guarded batch actions share one transaction-safe accounting path."
    echo "Directors can open full-row Athanor and person workspaces, then preview exact member scope."
    echo "Bills support reversible waivers, corrections, and voids with permanent reasons and history."
    echo "Every Moon Tax data table has accessible text, date, and numeric column sorting."
    echo "The overview now combines all visible extraction bills into billing-account totals."
