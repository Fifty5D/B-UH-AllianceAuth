...
release_script="${repo_root}/releases/moon-tax/v0.3.2/buh-moon-tax-platform-remote-v0.3.2.sh"

bash -n "${entry}" "${root_helper}" "${bootstrap}" "${release_script}"
...
grep -Fq 'install -o root -g root -m 0644 "${wheel}"' "${root_helper}"
grep -Fq 'install -o root -g root -m 0644 "/tmp/${TAX_WHEEL}"' "${release_script}"
