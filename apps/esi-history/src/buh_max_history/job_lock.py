...
def _lock_name(purpose="public"):
    database = str(connection.settings_dict["NAME"])
    identity = database if purpose == "public" else database + ":" + purpose
    return "buh_archive_" + hashlib.sha256(identity.encode()).hexdigest()[:40]


def assert_public_archive_lock(purpose="public"):
    ...
    if session is not None:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT IS_USED_LOCK(%s) = CONNECTION_ID()", [_lock_name(purpose)]
            )
...
@contextmanager
def public_archive_lock(purpose="public"):
    ...
    if connection.vendor == "mysql":
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT GET_LOCK(%s, 0)", [_lock_name(purpose)])
        ...
        try:
            ...
            if acquired and connection.connection is session:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT RELEASE_LOCK(%s)", [_lock_name(purpose)])
        return
    ...
    descriptor = os.open(
        root / f".{purpose}-archive.lock", os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW, 0o600
    )
    try:
        try:
