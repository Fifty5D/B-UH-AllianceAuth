...
than rejects one bounded, same-second owner incident containing the exact
nickname retry and Discord 403/code 50013. Alliance Auth 5.2 can emit that
incident through both `WARNING/MainProcess` Celery records and
...
unpaired evidence, unrelated errors, role failures, and other-member failures
all fail closed. The bounded finding retains its phase, container, process,
logger formats, and guild in `HEALTH.json`. Candidate output and all new-worker
